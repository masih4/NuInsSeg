...
for counter = 1:length(imagej_zips)
    ...
    savepath_mask_stack = strcat(results_path,'stacked mask','\', strcat(erase(raw_imgs(counter).name,'.png'),'.mat'));
    ...
    savepath_labelmask = strcat(results_path,'label masks modify','\', strcat(erase(raw_imgs(counter).name,'.png'),'.tif'));
